...
.rtu-header {
    ...
    text-align: center; color: white; font-size: 34px; font-weight: bold; box-shadow: 0px 4px 12px rgba(0,0,0,0.3);}
.rtu-overlay {background-color: rgba(0,0,0,0.55); padding: 40px; border-radius: 12px;}
.stButton button {background-color: #0B4DBA; color: white; border-radius: 10px; border: none;}
.stButton button:hover {background-color: #083b8a; color: white;}
...
# =========================
st.markdown("""
<div class="rtu-header"><div class="rtu-overlay">RTU Student Face Recognition System</div></div>
""", unsafe_allow_html=True)

st.markdown("""
<div style="font-size:18px;font-weight:700;color:white;text-align:center;margin-top:10px;
background-color:#0B4DBA;padding:10px;border-radius:8px;">
...
# CAMERA SECTION
with col1:
    st.markdown("""<div style="background-color:#0B4DBA;padding:10px;border-radius:8px;text-align:center;">
    <h2 style="color:white;margin:0;">📷 Camera</h2></div>""", unsafe_allow_html=True)
...
# PROFILE SECTION
with col2:
    st.markdown("""<div style="background-color:#FFD700;padding:10px;border-radius:8px;text-align:center;">
    <h2 style="color:#2c3e50;margin:0;">👤 Profile Information</h2></div>""", unsafe_allow_html=True)

    if st.session_state.captured_image is not None:
        if st.session_state.recognized_profile == "Unknown":
            st.warning("🚫 Not a student / Not registered")
...
<div style="display:flex; align-items:center; margin:20px 0;">
    <span style="font-size:25px; font-weight:bold; color:#2c3e50;">
        👤 {profile.get('Student_Name','Unknown')}
    </span>
    <span style="background-color:#2ecc71; color:white; padding:6px 14px;
...
""", unsafe_allow_html=True)
            ...
            with col_info:
                st.markdown(f"""
                        <div style="text-align:left;font-size:20px;color:#2c3e50;background-color:#f0f0f0;
            ...
            st.divider()
            st.markdown("""<div style="background-color:#e74c3c;padding:8px;border-radius:8px;text-align:center;">
            <h3 style="color:white;margin:0;">⚠️ Violation Records</h3></div>""", unsafe_allow_html=True)
